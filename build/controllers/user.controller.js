...
exports.activateUser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const parentData = await user_model_1.default.findOne({
            user_invite: parent_invt,
        });
        const grand_parent_invt = parentData ? parentData?.parent_invt : "";
        const grandParentData = await user_model_1.default.findOne({
            user_invite: parentData?.parent_invt,
        });
        const great_grandParentData = await user_model_1.default.findOne({
        ...
        await user_model_1.default.updateOne({ _id: parentData?._id }, { $push: { firstLevelData: user._id } });
        await user_model_1.default.updateOne({ _id: grandParentData?._id }, { $push: { secondLevelData: user._id } });
        await user_model_1.default.updateOne({ _id: great_grandParentData?._id }, { $push: { thirdLevelData: user._id } });
        res.status(201).json({
            success: true,
        });
    }
